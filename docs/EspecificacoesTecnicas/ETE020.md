...
7. Quando o usuário selecionar a opção “Nota Fiscal Eletrônica” no campo “Tipo de documento”, o sistema deve apresentar o campo “NFe (XML), permitir realizar o upload do XML da NFe e carregar automaticamente os campos: Nº Documento, Data do documento, Valor do documento e Fornecedor; 
8. Quando o usuário acionar a opção “Excluir” o arquivo XML da NFe, o sistema deve apresentar a mensagem de alerta ao usuário. Caso confirme a ação, apaga as informações carregadas automaticamente e permanece na tela de cadastro da entrada, bem como se a ação não for confirmada; [MSG032](DocumentoDeMensagensv2.md#msg032) 
9. O sistema deve validar a data do documento informada: 
...
11. Quando o usuário informar o CNES ou CNPJ do fornecedor e acionar a consulta, o sistema deve:  
...
12.  O campo “Data do Recebimento” dever ser preenchido automaticamente com a data atual e habilitado para edição;
13.  O sistema deve validar a data de recebimento informada: 
...
14.  O campo “Data do Armazenamento” dever ser preenchido automaticamente com a data atual e desabilitado para edição; 
15.  O campo “Em Análise” pode ser selecionado enquanto o(s) produtos da entrada não for(em) armazenado(s). Quando for selecionado, o sistema desabilita o botão “Armazenar”; 
16.  Quando o usuário acionar a opção de “Salvar”, o sistema deve verificar se: 
...
17.  Quando o usuário acionar a opção “Limpar”, o sistema deve apresentar a mensagem de alerta ao usuário. Caso confirme a ação, limpar os campos preenchidos e permanecer na tela de cadastro da entrada. Caso a ação não seja confirmada, não limpar os dados informados e permanecer na tela de cadastro da entrada; [MSG065](DocumentoDeMensagensv2.md#msg065) 
18.  Quando o usuário acionar a opção de “Voltar” ou “Consultar”, o sistema deve apresentar a mensagem de alerta ao usuário. Caso confirme a ação, retornar à tela de consulta as entradas e não salvar as edições realizadas. Caso a ação não seja confirmada, permanece na tela de cadastro da entrada; [MSG006](DocumentoDeMensagensv2.md#msg006)
19.  O sistema deve gravar a data, hora e CPF e nome do usuário que a executou qualquer ação de alteração no estado do registro. 
...
## Critérios de aceite
1. Quando o usuário realizar o upload do XML da NFe, o sistema deve: 
    * Identificar o(s) produto(s) da nota fiscal, recuperá-lo(s) na base de dados de medicamentos ou produtos para saúde; 
    * Apresentar em ordem alfabética a lista expansiva do(s) produto(s) recuperado(s) com o respectivo fabricante; [RGN023](DocumentoDeRegrasv2.md#rgn023)
    * Recuperar e apresentar o Nº do Lote e a Data de Validade e permitir o preenchimento dos demais campos de detalhamento do produto.
2. Quando o usuário acionar a opção “Excluir” o arquivo XML da NFe, o sistema deve apresentar a mensagem de alerta ao usuário. Caso confirme a ação, apaga as informações carregadas automaticamente e permanece na tela de cadastro da entrada, bem como se a ação não for confirmada; [MSG032](DocumentoDeMensagensv2.md#msg032)
3. Quando o usuário realizar a busca de um produto e selecioná-lo, o sistema deve incluí-lo  a lista de produtos adicionados à entrada por ordem alfabética. A lista será expansiva, permitindo o preenchimento dos campos de detalhamento do produto; [RGN023](DocumentoDeRegrasv2.md#rgn023)
4. O(s) produto(s) adicionado(s) à entrada pode(m) ser excluído(s) desde que não tenha(m) detalhamento(s) vinculado(s) a ele; [RGN025](DocumentoDeRegrasv2.md#rgn025)
5. O campo “Qtd Total” deve ser desabilitado para edição e o sistema deve:  
...
9. No campo “Valor Unitário” no detalhamento do Produto, o sistema deve: 
...
10. No campo “Programa de Saúde” no detalhamento do Produto, o sistema deve apresentar as opções:
...
11. No campo “Condições Especiais de Armazenamento” no detalhamento do Produto, o sistema deve apresentar as opções:
...
12. O campo “Descrição” somente será apresentado quando no campo “Condições Especiais de Armazenamento” for selecionada a opção “Outros”.  
13. No campo “Endereçamento Físico” no detalhamento do Produto, o sistema deve: 
...
14. Os Campos “CNES ou CNPJ” e “Razão Social” do Fabricante, no detalhamento do Produto, somente serão apresentados quando o produto adicionado for um Produto para a Saúde [(Descrição do Produto) + (CATMAT)]; 
15. Quando o usuário informar o CNES ou CNPJ do fabricante e acionar a consulta, o sistema deve: 
    * Validar o CNES ou CNPJ. Caso o dado seja inválido, o sistema deve realçar o campo e alertar ao usuário; [MSG015](DocumentoDeMensagensv2.md#msg015)
...
16. O sistema deve permitir adicionar mais de um detalhamento ao produto, permitindo a entrada do mesmo produto com lotes e datas de validades diferentes; 
17. O(s) detalhamento(s) vinculado(s) ao produto pode(m) ser editado(s), excluído(s) e detalhado(s); 
18. O sistema não deve permitir adicionar um detalhamento com o mesmo Nº do Lote e Data de Validade de um detalhamento já vinculado o produto na entrada; [MSG027](DocumentoDeMensagensv2.md#msg027)
19. Quando o usuário acionar a opção “Incluir”, o sistema deve gravar os dados do detalhamento e os apresentar na lista contendo: Nº do Lote, Data de Validade, Qtd Recebida e Programa de Saúde, com as ações “Editar”, “Excluir” e “Detalhar” para cada registro; 
20. Quando o usuário acionar a opção “Limpar”, o sistema deve limpar os campos preenchidos no detalhamento; 
21. Quando o usuário acionar a opção “Editar” o registro de um detalhamento, o sistema carrega os campos com os dados informados anteriormente e permite a edição; 
22. Quando o usuário acionar a opção “Excluir” o registro de um detalhamento, o sistema deve excluí-lo da lista de detalhamento(s) do produto; [RGN016](DocumentoDeRegrasv2.md#rgn016)
23. Quando o usuário acionar a opção “Detalhar” o registro de um detalhamento, o sistema deve apresentar os dados do referido registro; [Protótipo 004](ETE020.md#protótipo-004)
24. Quando o usuário acionar a opção “Armazenar”, o sistema deve verificar se: 
...
25. O sistema, identificando que uma entrada de produto(s) foi armazenada, deve creditar a quantidade no saldo deste(s), considerando lote, validade e programa de saúde e atualizar a posição de estoque do estabelecimento que realizou a entrada; [RGN031](DocumentoDeRegrasv2.md#rgn031)
26. Quando o usuário acionar a opção de “Salvar Parcial”, o sistema deve verificar se: 
...
27. Quando o usuário acionar a opção de “Voltar” ou “Consultar”, o sistema deve apresentar a mensagem de alerta ao usuário. Caso confirme a ação, retornar à tela de consulta as entradas e não salvar as edições realizadas. Caso a ação não seja confirmada, permanece na tela de cadastro da entrada; [MSG006](DocumentoDeMensagensv2.md#msg006)
28. O sistema deve gravar a data, hora e CPF e nome do usuário que a executou qualquer ação de alteração no estado do registro. [RGN005](DocumentoDeRegrasv2.md#rgn005)
