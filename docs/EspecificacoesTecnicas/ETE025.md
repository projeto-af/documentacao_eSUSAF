...
### Descrição resumida 
Editar saída/Dados da saída- Como usuário quero editar os dados gerais de uma movimentação de saída que se apresenta na situação “Em preenchimento”. 

[Inserir imagem] <!-- ![alt text](../imagens/ete-025-prot-001.png) -->

### Acesso 
Menu lateral > Movimentação > Saída > Consultar saída > Ação Editar 
...
5. O campo “Data da Saída” dever ser preenchido automaticamente com a data atual e desabilitado para edição; 
...
8. No campo “Justificativa” o sistema deve apresentar as opções: Ajuste de estoque, Demanda ordinária, Descarte ou devolução a fornecedores, Devolução e empréstimos, Doação, Outros, Quebra do produto/medicamento, Recebimento maior que a demanda, Recebimento próximo da validade, Roubo, Sistema inoperante e Transferência.  <!-- "Já está em “Campos” " -->
...
10. Quando o usuário acionar a opção “Limpar”, o sistema limpa os campos preenchidos e permanecer na tela de cadastro da saída.   
11. Quando o usuário acionar a opção de “Voltar” ou “Consultar”, o sistema retorna à tela de consulta as saídas e não salvar as edições realizadas.  
...
### Descrição resumida 
Editar saída/Relação de produtos da saída - Como usuário quero editar os dados do(s) produto(s) de uma movimentação de saída que se apresenta na situação “Em preenchimento”. 

[Inserir imagem] <!-- ![alt text](../imagens/ete-025-prot-002.png) -->

### Acesso 

Menu lateral > Movimentação > Saída > Consultar saída > Ação Editar> Botão Salvar 

### Campos 
- Produto - obrigatório <!-- Estava com indentação no documento de origem -->
- Quantidade a expedir - obrigatório <!-- Estava com indentação no documento de origem -->
...
4. O sistema deve recuperar e apresentar os dados da Saída cadastrada previamente, permitindo a edição de todos os campos;  
5. Quando o usuário buscar por um produto, o sistema só lista produtos que possuam estoque no estabelecimento logado. 
...
6. O sistema deve apresentar a lista de produtos adicionados à saída por ordem alfabética. A lista será expansiva, permitindo o preenchimento dos campos de detalhamento do produto; [RGN023](DocumentoDeRegrasv2.md#rgn023) 
7. O(s) produto(s) adicionado(s) à saída pode(m) ser excluído(s) desde que não tenha(m) detalhamento(s) vinculado(s) a ele; [RGN035](DocumentoDeRegrasv2.md#rgn035) 
8. O campo “Qtd Estoque Atual” deve ser desabilitado para edição e carregado automaticamente com o saldo do produto em estoque do estabelecimento logado, no momento, independente de lote, validade, fabricante, programa de saúde e endereçamento físico; [RGN036](DocumentoDeRegrasv2.md#rgn036) 
9. O campo “Qtd a Expedir” deve ser desabilitado para edição e o sistema deve: 
      ...
      * Calcular automaticamente somando todas as quantidades a expedir informadas nos detalhamentos adicionados ao produto; [RGN039](DocumentoDeRegrasv2.md#rgn039) <!-- No documento de origem informa: "Não consegui reproduzir!!!! " -->
10. O campo “Qtd Expedida” deve ser desabilitado para edição e o sistema deve: 
...
14. O(s) detalhamento(s) vinculado(s) ao produto pode(m) ser excluído(s); 
15. Quando o usuário acionar a opção “Registrar”, o sistema deve verificar se:  
