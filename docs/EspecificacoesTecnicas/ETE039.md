...
O usuário do sistema poderá filtrar por: 

* Data: campo para seleção de data  
* Nº de transferência: ID de transferência 
* Produto: pesquisa do medicamento pelo princípio ativo 
* Status:  
    a) Em separação  
    b) Aguardando transporte  
    c) Em Transito  
    d) Entregue  
    e) Armazenado  
    f) Aceito parcialmente 
     <!-- Faltam imagens na tabela abaixo: >

|ID|Est. De Origem|Est.de Destino|Solicitado em|Status|Ações|
|--|--------------|--------------|-------------|------|-----| 
|  |              |              |             |      | Detalhar |    
