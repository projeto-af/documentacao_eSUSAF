...
3. A persistência dos dados para auditoria deve ocorrer somente no caso do campo “Tempo guarda envios (dias)” da [ESPECIFICAÇÃO TÉCNICA 041: Configurar transmissão SI Bnafar](ETE041.md) estar preenchido com “Sim”. Somente deverão ser auditados as requisições referentes ao envio de dados (consultas não serão incluídas).
...
5. O sistema deve possuir uma rotina para excluir os registros de auditoria com o tempo maior de persistência do que especificado no “Auditar transmissões” -> “Dias” no [#ESPECIFICAÇÃO TÉCNICA 041: Configurar transmissão SI Bnafar](ETE041.md). Exclusão deverá ser física. [#RGN070](DocumentoDeRegrasv2.md).  
