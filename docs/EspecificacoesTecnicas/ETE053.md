# ETE053- Cadastrar dispensação/fornecimento 

## Descrição
Como usuário, desejo cadastrar uma nova dispensação/fornecimento de produto(s) ao Usuário SUS, no estabelecimento de saúde logado. 

## Protótipo 001  
![alt text](../imagens/ETE053-Prot1-Cadastrar-dispensacao-fornecimento.png)
![alt text](../imagens/ETE053-Prot1-2-Cadastrar-dispensacao-fornecimento.png)
...
3. Quando o usuário acessar a funcionalidade, o sistema deve apresentar a tela contendo os seguintes campos: 
    ...
    - Dados do Usuário SUS: 
        - CPF/CNS: Preenchendo o número do documento, as demais informações do Usuário SUS serão buscadas na base de dados do sistema (banco local), ou da integração com o CADSUS (banco nacional), que irá atualizar a base local:  
        ...
        - Nome Completo: será preenchido de forma automática após a informação do CPF/CNS;  
        - Nome Social: será preenchido de forma automática após a informação do CPF/CNS, se aplicável;  
        - Sexo biológico/nascimento: será preenchido de forma automática após a informação do CPF/CNS;  
        - Data de Nascimento: será preenchido de forma automática após a informação do CPF/CNS 
            - Identificação do Responsável pela Retirada: 
        - Retirado por: as opções listadas serão “Usuário SUS”, “Responsável Legal”, “Outros”:  
        ...
        - CPF/CNS: Quando selecionado em “Retirado por” as opções “Usuário SUS” ou “Responsável Legal”, o número do documento será preenchido de forma automática, pela busca às bases:  
            - Se for selecionada a opção “Outros”, um dos documentos (CPF/CNS) deverá ser digitado. 
        - Nome Completo: Quando selecionado em “Retirado por” as opções “Usuário SUS” ou “Responsável Legal”, o campo será preenchido de forma automática, pela busca às bases:  
...
## Protótipo 002 
![alt text](../imagens/ETE053-Prot2-Cadastrar-dispensacao-fornecimento.png)
![alt text](../imagens/ETE053-Prot2-2-Cadastrar-dispensacao-fornecimento.png)
...
4. Quando o usuário acessar a funcionalidade, o sistema deve apresentar a tela contendo os seguintes campos: 
    - Data da Receita/Prescrição: Será informada a data que consta na receita/prescrição do medicamento. O operador poderá digitar os números, ou clicar no ícone do calendário que fica à direita, para preenchimento deste campo.  
    ...
    - Registro Eletrônico: No caso de prescrições eletrônicas, neste campo será informado o token/código da receita. Este campo não é obrigatório, no momento, e deve ser no formato alfanumérico.  
    - CPF/CNS: Neste, deverá ser informado o documento do prescritor.  
    - Nome Completo: Deverá ser preenchido o nome completo do prescritor, sem abreviações. Campo obrigatório.  
    - Conselho de classe: Campo onde será selecionado em qual conselho de classe o prescritor está inscrito, dentre as opções:  
    ...
    - Número do Conselho: Deverá ser digitado o número de registro do conselho de classe ao qual o prescritor está inscrito.  
    - UF do Conselho: Campo para selecionar em qual unidade federativa o prescritor está inscrito em seu conselho de classe.  
    - Número do RQE: Quando o profissional possuir especialidade médica, nomeado como Registro de Qualificação de Especialista (RQE), deverá ser digitado o número deste registro.  
    - Especialidade médica: Quando informado um RQE, este campo se tornará obrigatório, onde deverá ser selecionada uma especialidade médica dentre as 55 opções disponíveis (RESOLUÇÃO CFM nº 2.380, DE 18 DE JUNHO DE 2024).  
    - CNES: Deverá ser informado o número do Cadastro Nacional de Estabelecimentos de Saúde onde ocorreu a prescrição.  
        - Se o número do CNES informado for inválido, o sistema irá acusar com mensagem. 
    - UF do Estabelecimento: após a informação do CNES, essa informação é carregada automaticamente ao clicar na lupa.  
    - Nome do Estabelecimento: após a informação do CNES, essa informação é carregada automaticamente ao clicar na lupa.  
    - Tipo: Será necessário selecionar uma das opções entre CID-10 ou CIAP.  
    - Condição Clínica: De acordo com a seleção do “Tipo”, esse campo permitirá a busca pelo código da CID-10 ou CIAP ou ainda, pelo nome da condição clínica.  
...
## Protótipo 003 
![alt text](../imagens/ETE053-Prot3-1-Cadastrar-dispensacao-fornecimento.png)
![alt text](../imagens/ETE053-Prot3-2-Cadastrar-dispensacao-fornecimento.png)
![alt text](../imagens/ETE053-Prot3-3-Cadastrar-dispensacao-fornecimento.png)
...
4.  Ao selecionar o produto desejado, irá abrir um modal com as informações do produto, contendo os seguintes campos: 
    - Produto: Reporta o nome completo do produto escolhido, contendo o princípio ativo e sua concentração. Ex.: Isoniazida 150mg + Rifampicina 300mg. 
    ...
    - Apresentação: Como alguns produtos são fabricados na mesma concentração em diferentes formas farmacêuticas, esta informação se torna importante para a conferência do produto a ser dispensado. Este dado está atrelado ao cadastro do produto (movimentação de entrada). Assim, quando o produto é selecionado no campo anterior, essa informação será replicada da base. 
    - Status: Esse campo irá informar três situações que podem ocorrer no atendimento da demanda: “Não Atendido”, “Atendimento parcial” e “Atendimento total”. 
    ...
    - Quantidade prescrita: Deverá ser informada a quantidade prescrita do produto, em função da posologia e do tempo de tratamento. Esse cálculo fica a cargo do farmacêutico ou atendente. Deverá ser informada sempre a menor unidade de distribuição (ex.: por comprimido, frasco, bisnaga, ampola, etc.).  
    - Quantidade dispensada/fornecida: Esse campo será automaticamente preenchido (não permite digitação) de acordo com a informação fornecida pelo operador no campo “Quantidade dispensada/fornecida” relacionada ao lote específico. Ela nunca poderá ser maior que a quantidade informada no campo “Quantidade prescrita”. 
    ...
    - Quantidade dispensada/fornecida: Esse campo se repete na tela. Neste caso campo é digitável, permitindo que o operador insira um valor que não ultrapasse a “Quantidade estoque”, e seja menor ou igual à “Quantidade prescrita”. 
    ...
    - Justificativa: Esse campo texto só irá ser obrigatório quando o Status estiver como “Não atendido” e “Atendimento parcial”. 
