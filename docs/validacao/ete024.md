...
3.1. O atributo "Departamento" deverá estar visível uma vez selecionado o tipo de saída "Saída para Departamento";  
3.1.1. O combo de Departamentos deverá ser populado com a relação de departamentos previamento cadastrados pelo estabelecimento;  
3.1.2. O cnes de destino deverá ser preenchido com o cnes do estabelecimento que está realizando a movimentação;    
3.2. Uma vez selecionado o tipo de saída "Distribuição sem Requisição" ou "Transferência", o usuário deverá preencher o atributo CNES/CNPJ de destino;  
3.2.1. O CNES de destino informado deverá ser diferente do CNES do estabelecimento atendente.  
3.3. Para os tipos de saída "Validade Vencida", "Ajuste de estoque", "Amostra", "Exposição e Análise", "Apreensão Sanitária", "Perda", "Roubo" e "Validade Vencida", deverão ser preenchidos os atributos "CNES/CNPJ do Estabelecimento de Destino", "Tipo de documento", "Nº do documento" e "Data do documento";  
3.3.1. Para quaisquer dos tipos de saída citados no item 3.3, a saída poderá ter como destino o próprio estabelecimento que realiza a saída.  
3.4. Para o tipo de saída "Usuário SUS", nenhum dos campos adicionais será necessário.  
3.5. Quando o tipo de saída for "Validade Vencida", apenas produtos com até 90 dias a expirar poderão ser utilizados para expedir a saída.  
4. Não será permitido o relacionamento de um produto já relacionado à saída;
5. Deve existir ao menos um produto relacionado à saída;
6. A saída e seus itens poderão ser excluídos enquanto a saída estiver em status "Em preenchimento".
7. O sistema deve apresentar a lista de produtos adicionados à saída por ordem alfabética. A lista será expansiva, permitindo o preenchimento dos campos de detalhamento do produto;
8. Quando o usuário acionar a opção de “Finalizar Saída”, o sistema deve verificar se:
8.1. O detalhamento de todos os produtos adicionados foi incluído. Caso não, o sistema deve apresentar uma mensagem de erro ao usuário;  
8.2. Verificar se para todos os itens solicitados há estoque disponível, caso não, deverá retornar mensagem ao usuário informando de que não há estoque suficiente para um dado item.  
8.3. Após realizar a baixa dos itens em estoque, deverá retornar mensagem de sucesso, alterar o status da saída para finalizado e incluir a data da saída. 
9. O usuário poderá imprimir a saída conforme modelo apresentado.
10. O Usuário poderá exportar a saída em formato CSV. 
