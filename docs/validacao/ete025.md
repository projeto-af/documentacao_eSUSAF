...
4.1. O atributo "Departamento" deverá estar visível uma vez selecionado o tipo de saída "Saída para Departamento";  
4.1.1. O combo de Departamentos deverá ser populado com a relação de departamentos previamento cadastrados pelo estabelecimento;  
4.1.2. O cnes de destino deverá ser preenchido com o cnes do estabelecimento que está realizando a movimentação;  
4.2. Uma vez selecionado o tipo de saída "Distribuição sem Requisição" ou "Transferência", o usuário deverá preencher o atributo CNES/CNPJ de destino;  
4.2.1. O CNES de destino informado deverá ser diferente do CNES do estabelecimento atendente.  
4.3. Para os tipos de saída "Validade Vencida", "Ajuste de estoque", "Amostra", "Exposição e Análise", "Apreensão Sanitária", "Perda", "Roubo" e "Validade Vencida", deverão ser preenchidos os atributos "CNES/CNPJ do Estabelecimento de Destino", "Tipo de documento", "Nº do documento" e "Data do documento";  
4.3.1. Para quaisquer dos tipos de saída citados no item 3.3, a saída poderá ter como destino o próprio estabelecimento que realiza a saída.  
4.4. Para o tipo de saída "Usuário SUS", nenhum dos campos adicionais será necessário.  
4.5. Quando o tipo de saída for "Validade Vencida", apenas produtos com até 90 dias a expirar poderão ser utilizados para expedir a saída.  
...
9.1. O detalhamento de todos os produtos adicionados foi incluído. Caso não, o sistema deve apresentar uma mensagem de erro ao usuário;  
9.2. Verificar se para todos os itens solicitados há estoque disponível, caso não, deverá retornar mensagem ao usuário informando de que não há estoque suficiente para um dado item.  
9.3. Após realizar a baixa dos itens em estoque, deverá retornar mensagem de sucesso, alterar o status da saída para finalizado e incluir a data da saída.  
